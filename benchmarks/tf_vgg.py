"""
tensorflow vgg-style cnn benchmark -- mirrors benchmarks/bench_vgg.c exactly.

architecture (~8M params):
  block 1: conv(1,64,3,pad=same) bn relu conv(64,64,3,pad=same) bn relu maxpool(2)
  block 2: conv(64,128,3,pad=same) bn relu conv(128,128,3,pad=same) bn relu maxpool(2)
  block 3: conv(128,256,3,pad=same) bn relu conv(256,256,3,pad=same) bn relu maxpool(2)
...
def read_idx_images(path, n):
    ...
    return arr.reshape(n, 28, 28, 1)  # NHWC for tf
...
def run_train():
    ...
    print(f"=== tf vgg train ({device_label}) ===")
    if not USE_GPU:
    ...
    print(f"model: vgg-mnist  params: {n_params}")
    ...
    for ep in range(EPOCHS):
        ...
        _, acc = model.evaluate(test_x, test_y, verbose=0, batch_size=256)
    ...
    _, final_acc = model.evaluate(test_x, test_y, verbose=0, batch_size=256)

    print(f"\n--- tf vgg {device_label} summary ---")
...
def run_infer():
    device_label = "gpu" if USE_GPU else "cpu"
    print(f"=== tf vgg infer ({device_label}) ===")
    if not USE_GPU:
        print(f"threads (intra_op): {NUM_THREADS}")
    ...
    print(f"--- tf vgg {device_label} forward summary ---")
