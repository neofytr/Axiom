/* ax_mnist_gpu.c — mnist mlp training on gpu via cuda backend.
   ...
   data flow: load on cpu -> model on cuda -> per-batch h2d -> train on gpu.
   eval copies logits d2h per batch for argmax on cpu. */
...
int main(void) {
    ...
    /* model created on gpu (weights initialized via device-safe init) */
    ...
    for (int ep = 0; ep < epochs; ep++) {
        ...
        while (ax_dataloader_next(dl, &b)) {
            ...
            ax_set_default_device(AX_DEVICE_CUDA);
#else
            ax_tensor_t *gi = b.input;
            ax_tensor_t *gt = b.target;
#endif
            ax_enable_grad();
